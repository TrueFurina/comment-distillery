...
> Website: <https://truefurina.github.io/comment-distillery/>

### 1. Desktop app (Windows, GUI)

If you'd rather not touch a terminal:

**[⬇ Download comment-distillery.exe](https://github.com/TrueFurina/comment-distillery/releases/latest)** — ~11 MB, single file, no installer.

Four tabs, in order:

| Tab | What it does |
|---|---|
| **1. Collect** | Bilibili link → comments (incl. nested replies) → CSV; can stop midway and still writes what it got |
| **2. Prep** | dedupe / denoise / stats. **Check `dup_dropped` before trusting any scale number** |
| **3. Export pack** | one click → a folder with corpus + methodology + a paste-ready `PROMPT.md` |
| **4. Verify** | hallucinated-citation set-difference check — a non-empty difference means no delivery |

**The distillation step is done by the AI you already use** (Claude / ChatGPT / Codex / WorkBuddy…), so **no API key is needed** and there is no per-call cost. The methodology stays in sync with the `SKILL.md` you have — it can't go stale.

> The exe is unsigned; Windows SmartScreen will prompt on first run — that's expected, not malware.
> Build it yourself: `python scripts/make_icon.py && python scripts/build_exe.py`.

### 2. Install into your agent (one command)
...
**Zero dependencies — no `pip install` needed.** Python 3.10+ is enough.
...
| `corpus_ccf.csv` | **canonical corpus** (Canonical Corpus Format) — the single source of truth for citation checks and cross-corpus merges |
...
├── app_main.py              # desktop entry (source run / PyInstaller entry)
├── app/                     # ★ desktop app (tkinter, zero third-party deps)
│   ├── core.py              #   logic layer: delegates to scripts/ & contrib/; no GUI
│   └── gui.py               #   UI layer: the only module that touches the GUI
├── site/index.html          # website (single static file, published via GitHub Pages)
├── assets/icon.ico          # app icon (generated by scripts/make_icon.py, stdlib only)
├── docs/
...
└── tests/                   # regression tests, 61 cases (run in CI)
```
