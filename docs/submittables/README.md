...
> 这些是**最终成稿**，不是草稿。每一篇都可以整篇复制粘贴，不需要再改写。
> 提交动作本身必须由你本人执行（涉及以你的账号公开发言）。
...
**Reddit**：`reddit.com` → 右上 Create Post → 选对应社区（r/ClaudeAI / r/LocalLLaMA）→ 贴标题、贴正文（Markdown 格式直接贴）→ Post。
发完**隔 1 小时回来看评论**，评论回复也属公开发言，回复口径参考各稿末尾的「Ask」段。
...
- **不回评推广**：只回答别人的技术问题；有人质疑就引 `docs/retrospective.md` 的实测数字。
- **不自夸**：三篇中文稿全部是「被数据打穿」的复盘口吻，Reddit 稿主动交代局限——这是刻意设计，别改成营销文案。
- **被拒别重发**：Reddit 删帖/被拒就记录到本文件状态列，不换号重发。
- 提交后把上表状态改为 ✅ 并附链接，提交 `docs/distribution.md` 相应行。
